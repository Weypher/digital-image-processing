...

{
    ...
    //denoising by uniform weight function
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            int n = i + 2;
            int m = j + 2;
            ImageRes[i][j][0] = (unsigned char)(((float)fixedBoundPic[n - 1][m - 1][0] + (float)fixedBoundPic[n - 1][m][0]
                                                 + (float)fixedBoundPic[n - 1][m + 1][0] + (float)fixedBoundPic[n][m - 1][0]
                                                 + (float)fixedBoundPic[n][m][0] + (float)fixedBoundPic[n][m + 1][0]
                                                 + (float)fixedBoundPic[n + 1][m - 1][0] + (float)fixedBoundPic[n + 1][m][0]
                                                 + (float)fixedBoundPic[n + 1][m + 1][0]) / 9);
        }
    }
    ...
    cout << "Uniform weight filter : " << PSNR << endl;
