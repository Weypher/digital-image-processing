...

{
    ...
    unsigned char ImageOri[height][width][BytesOutPut];
    ...
    for (int i = 0; i < height; i++) {
        for (int j = 0; j < width; j++) {
            int n = i + 2;
            int m = j + 2;
            ImageRes[i][j][0] = (unsigned char)(((float)fixedBoundPic[n - 2][m - 2][0] * 1 + (float)fixedBoundPic[n - 2][m - 1][0] * 4
                                                 + (float)fixedBoundPic[n - 2][m][0] * 7 + (float)fixedBoundPic[n - 2][m + 1][0] * 4
                                                 + (float)fixedBoundPic[n - 2][m - 2][0] * 1 + (float)fixedBoundPic[n - 1][m - 2][0] * 4
                                                 + (float)fixedBoundPic[n - 1][m - 1][0] * 16 + (float)fixedBoundPic[n - 1][m][0] * 26
                                                 + (float)fixedBoundPic[n - 1][m + 1][0] * 16 + (float)fixedBoundPic[n - 1][m + 2][0] * 4
                                                 + (float)fixedBoundPic[n][m - 2][0] * 7 + (float)fixedBoundPic[n][m - 1][0] * 26
                                                 + (float)fixedBoundPic[n][m][0] * 41 + (float)fixedBoundPic[n][m + 1][0] * 26
                                                 + (float)fixedBoundPic[n][m + 2][0] * 7 + (float)fixedBoundPic[n + 1][m - 2][0] * 4
                                                 + (float)fixedBoundPic[n + 1][m - 1][0] * 16 + (float)fixedBoundPic[n + 1][m][0] * 26
                                                 + (float)fixedBoundPic[n + 1][m + 1][0] * 16 + (float)fixedBoundPic[n + 1][m + 2][0] * 4
                                                 + (float)fixedBoundPic[n + 2][m - 2][0] + (float)fixedBoundPic[n + 2][m - 1][0] * 4
                                                 + (float)fixedBoundPic[n + 2][m][0] * 7 + (float)fixedBoundPic[n + 2][m + 1][0] * 4
                                                 + (float)fixedBoundPic[n + 2][m + 2][0]) / 273);
        }
    }
    ...
    cout << "Gaussian weight filter : " << PSNR << endl;
